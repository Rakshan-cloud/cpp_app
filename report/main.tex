...
\author{\IEEEauthorblockN{Rakshan}
\IEEEauthorblockA{Student ID: 25180754 \\
MSc in Cloud Computing \\
National College of Ireland \\
Dublin, Ireland \\
25180754@student.uwa.edu.au}}
...
The library is published on the Python Package Index (PyPI) at \url{https://pypi.org/project/qr-ticket-manager/} and can be installed via \texttt{pip install qr-ticket-manager}. The source code is maintained within a private GitHub repository at \url{https://github.com/Rakshan-cloud/cpp\_app}.
...
A CI/CD pipeline was implemented using GitHub Actions, triggered on every push to the main branch of the private repository. The pipeline consists of three sequential stages that ensure code quality before deployment. The first stage runs the custom library's test suite using pytest, executing fifteen unit tests that validate ticket generation, signature verification, QR code creation, and all failure modes. Only when all tests pass does the pipeline proceed to deployment. The second stage packages the Flask backend with the custom library and deploys it to Elastic Beanstalk by uploading the application bundle to S3 and creating a new application version. The third stage builds the React frontend with production environment variables injected at build time and synchronises the compiled assets to the S3 hosting bucket. All sensitive configuration values including AWS credentials are stored as encrypted GitHub Secrets and never exposed in the repository or build logs. This pipeline embodies the principles of continuous integration through automated testing, continuous delivery through automated packaging, and continuous deployment through automatic release to the production environment.

The deployed application is accessible at: \url{http://rakshan-frontend.s3-website-ap-southeast-2.amazonaws.com}. The backend API is hosted at: \url{http://rakshan-prod.eba-7mxtcx4w.ap-southeast-2.elasticbeanstalk.com}. The custom library is published at: \url{https://pypi.org/project/qr-ticket-manager/}.
...
The project could be extended in several directions for production readiness, including adding Amazon CloudFront as a CDN for HTTPS support, implementing infrastructure-as-code through AWS CloudFormation or Terraform to formalise the resource provisioning process, and integrating Amazon SNS for real-time push notifications when tickets are validated at event venues.
