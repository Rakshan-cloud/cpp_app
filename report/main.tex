...
The library is published on the Python Package Index (PyPI) at \url{https://pypi.org/project/qr-ticket-manager/} and can be installed via \texttt{pip install qr-ticket-manager}. The source code is maintained within the project repository at \url{https://github.com/Rakshan-cloud/cpp\_app}.
...
A CI/CD pipeline was implemented using GitHub Actions, triggered on every push to the main branch. The pipeline consists of three stages: testing the custom library with pytest, deploying the Flask backend to Elastic Beanstalk, and building and deploying the React frontend to S3. AWS credentials are stored as encrypted GitHub repository secrets, ensuring that no sensitive values are committed to the codebase. The project could be extended in several directions for production readiness, including adding Amazon CloudFront as a CDN for HTTPS support and implementing infrastructure-as-code through AWS CloudFormation or Terraform to formalise the resource provisioning process.
